...
import { GetOneProduct } from "@/types/products/getOneProduct";
import { GetAllProducts } from "@/types/products/getAllProducts";
import { CreateProduct } from "@/types/products/createProduct";
...
export const useGetOneBlog = (id: string) => {
    ...
    return useQuery({
        queryKey: ["blog", id],
        queryFn: async () => {
            const res = await apiClient.get<GetOneBlogType>(`/Blog/${id}`, { headers });
            return res.data;
        },
        enabled: !!id,
    });
...
export const useGetAllProducts = () => {
    ...
    return useQuery({
        queryKey: ["products"],
        queryFn: async () => {
            const res = await apiClient.get<GetAllProducts>("/Product", { headers });
            return res.data;
        },
    });
};
...
export const useGetOneProduct = (id: string) => {
    ...
    return useQuery({
        queryKey: ["product", id],
        queryFn: async () => {
            const res = await apiClient.get<GetOneProduct>(`/Product/${id}`, { headers });
            return res.data;
        },
        enabled: !!id,
    });
